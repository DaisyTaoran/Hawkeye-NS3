...
#include <ns3/myrdma-client-helper.h>
#include <ns3/myrdma-driver.h>
#include <ns3/myswitch-node.h>
...
void qp_finish(FILE* fout, Ptr<MyRdmaQueuePair> q){ // 处理 RDMA 队列对（Queue Pair, QP）的结束操作。
	...
	Ptr<MyRdmaDriver> rdma = dstNode->GetObject<MyRdmaDriver> ();
	rdma->m_rdma->DeleteRxQp(q->sip.Get(), q->m_pg, q->sport);
}

void get_pfc(FILE* fout, Ptr<MyCsmaNetDevice> dev, uint32_t type){ // 把pfc包的相关信息写入输出文件
...
void monitor_buffer(FILE* qlen_output, NodeContainer *n){ // 监控交换机的缓冲区（队列）状态，并将队列长度的分布情况记录到文件中。它定期收集交换机端口的队列长度数据，并写入文件
	for (uint32_t i = 0; i < n->GetN(); i++){                       // 更新所有交换机的队列长度分布，保存在 queue_result 中
		if (n->Get(i)->GetNodeType() == 1){ // is switch
			Ptr<MySwitchNode> sw = DynamicCast<MySwitchNode>(n->Get(i)); // 将节点动态转换为 MySwitchNode 类型，以便访问交换机的特定属性和方法。
		...
		for (auto j = table.begin(); j != table.end(); j++){ // 遍历 table 中的所有目标节点。
			...
			for (int k = 0; k < (int)nexts.size(); k++){
				...
				if (node->GetNodeType() == 1)                // 如果 node 是交换机：
					DynamicCast<MySwitchNode>(node)->AddTableEntry(dstAddr, interface); // 将目标 IP 地址和下一跳接口索引，添加到交换机的路由表中
				else{                                        // 如果 node 是主机：
					node->GetObject<MyRdmaDriver>()->m_rdma->AddTableEntry(dstAddr, interface); // 将目标 IP 地址和下一跳接口索引，添加到主机的路由表中
				}
			}
		}
	}
...
void TakeDownLink(NodeContainer n, Ptr<Node> a, Ptr<Node> b){ // 模拟网络中两个节点之间的链路失效（例如链路断开或故障）
	...
	for (uint32_t i = 0; i < n.GetN(); i++){        // 遍历所有节点，并清除路由表
		if (n.Get(i)->GetNodeType() == 1)
			DynamicCast<MySwitchNode>(n.Get(i))->ClearTable();
		else
			n.Get(i)->GetObject<MyRdmaDriver>()->m_rdma->ClearTable();
	}
	DynamicCast<MyCsmaNetDevice>(a->GetDevice(nbr2if[a][b].idx))->TakeDown(); // 通知设备 a 链路失效
	DynamicCast<MyCsmaNetDevice>(b->GetDevice(nbr2if[b][a].idx))->TakeDown(); // 通知设备 b 链路失效
	// reset routing table
	...
	for (uint32_t i = 0; i < n.GetN(); i++){
		if (n.Get(i)->GetNodeType() == 0)       // 遍历所有主机节点
			n.Get(i)->GetObject<MyRdmaDriver>()->m_rdma->RedistributeQp(); // 重新分配队列对（QP），以适应新的网络拓扑
	}
}
...
uint64_t get_nic_rate(NodeContainer &n){
	for (uint32_t i = 0; i < n.GetN(); i++)
		if (n.Get(i)->GetNodeType() == 0)
			return DynamicCast<MyCsmaNetDevice>(n.Get(i)->GetDevice(1))->GetDataRate().GetBitRate();
}
...
int main(int argc, char *argv[])
{
	...
	Config::SetDefault("ns3::MyCsmaNetDevice::PauseTime", UintegerValue(pause_time));
	Config::SetDefault("ns3::MyCsmaNetDevice::QcnEnabled", BooleanValue(enable_qcn));
	Config::SetDefault("ns3::MyCsmaNetDevice::DynamicThreshold", BooleanValue(dynamicth));
	...
	for (uint32_t i = 0; i < node_num; i++){ 
		if (node_type[i] == 0)
			...
			Ptr<MySwitchNode> sw = CreateObject<MySwitchNode>();
			n.Add(sw);
	...
	MyCsmaHelper csma;          // 用于创建和管理 MyCsmaNetDevice 和 MyCsmaChannel 的帮助类。
	Ipv4AddressHelper ipv4; // 用于分配和管理 IPv4 地址。
	for (uint32_t i = 0; i < link_num; i++) // 读取topo文件中链路信息，为每对节点创建网络设备（NetDevice）和信道（Channel），并设置相关的链路
	{
		uint32_t src, dst;
		...
		csma.SetDeviceAttribute("DataRate", StringValue(data_rate));     
		csma.SetChannelAttribute("Delay", StringValue(link_delay));      
		...
		NetDeviceContainer d = csma.Install(snode, dnode);               // 在 snode 和 dnode 之间安装网络设备和信道，并返回设备容器 d
		...
		nbr2if[snode][dnode].idx = DynamicCast<MyCsmaNetDevice>(d.Get(0))->GetIfIndex();
		nbr2if[snode][dnode].up = true;
		nbr2if[snode][dnode].delay = DynamicCast<MyCsmaChannel>(DynamicCast<MyCsmaNetDevice>(d.Get(0))->GetChannel())->GetDelay().GetTimeStep();
		nbr2if[snode][dnode].bw = DynamicCast<MyCsmaNetDevice>(d.Get(0))->GetDataRate().GetBitRate();
		nbr2if[dnode][snode].idx = DynamicCast<MyCsmaNetDevice>(d.Get(1))->GetIfIndex();
		nbr2if[dnode][snode].up = true;
		nbr2if[dnode][snode].delay = DynamicCast<MyCsmaChannel>(DynamicCast<MyCsmaNetDevice>(d.Get(1))->GetChannel())->GetDelay().GetTimeStep();
		nbr2if[dnode][snode].bw = DynamicCast<MyCsmaNetDevice>(d.Get(1))->GetDataRate().GetBitRate();
		...
		DynamicCast<MyCsmaNetDevice>(d.Get(0))->TraceConnectWithoutContext("MyCsmaPfc", MakeBoundCallback (&get_pfc, pfc_file, DynamicCast<MyCsmaNetDevice>(d.Get(0)))); // pfc.txt
		DynamicCast<MyCsmaNetDevice>(d.Get(1))->TraceConnectWithoutContext("MyCsmaPfc", MakeBoundCallback (&get_pfc, pfc_file, DynamicCast<MyCsmaNetDevice>(d.Get(1)))); // pfc.txt
	}
	...
	for (uint32_t i = 0; i < node_num; i++){
	        ...
	        if (snode->GetNodeType() == 1){ // 如果是switch，就为它和分析器建立链路
	                uint32_t src = i, dst = analysis_node;
                        // 设置点到点链路 
		        csma.SetDeviceAttribute("DataRate", StringValue("100Gbps"));    
		        csma.SetChannelAttribute("Delay", StringValue("0.001ms"));     
		        
		        fflush(stdout);
		        
                        // 安装网络设备
		        NetDeviceContainer d = csma.Install(snode, dnode);
		        if (snode->GetNodeType() == 0){                                 
			        Ptr<Ipv4> ipv4 = snode->GetObject<Ipv4>();
			        uint32_t in = ipv4->AddInterface(d.Get(0));                           
			        ipv4->AddAddress(in, Ipv4InterfaceAddress(serverAddress[src], Ipv4Mask(0xff000000)));
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // is switch
			Ptr<MySwitchNode> sw = DynamicCast<MySwitchNode>(n.Get(i));
			sw->m_analysis_addr = serverAddress[analysis_node];// TODO
			uint32_t shift = 3; // by default 1/8
			for (uint32_t j = 1; j < sw->GetNDevices(); j++){
			        Ptr<MyCsmaNetDevice> dev = DynamicCast<MyCsmaNetDevice>(sw->GetDevice(j));
			        if(dev == 0) continue; 
				// set ecn
				...
				// set pfc
				uint64_t delay = DynamicCast<MyCsmaChannel>(dev->GetChannel())->GetDelay().GetTimeStep();
	...
	for (uint32_t i = 0; i < node_num; i++){
		...
		if (n.Get(i)->GetNodeType() == 0){ // is host, not switch
			// create RdmaHw
			Ptr<MyRdmaHw> rdmaHw = CreateObject<MyRdmaHw>();
			...
			// create and install MyRdmaDriver
			Ptr<MyRdmaDriver> rdma = CreateObject<MyRdmaDriver>(); 
			Ptr<Node> node = n.Get(i);     
			rdma->SetNode(node);            
	...
	if (ack_high_prio)
		MyRdmaEgressQueue::ack_q_idx = 0;
	else
		MyRdmaEgressQueue::ack_q_idx = 3;
	...
	if (enable_trace){
		//csma.EnableTracing(trace_output, trace_nodes);   
                csma.EnablePcapAll("mixpcap/mypcap");
        }
	// dump link speed to trace file
	{
		...
		for (auto i: nbr2if){
			for (auto j : i.second){
				...
				uint64_t bps = DynamicCast<MyCsmaNetDevice>(i.first->GetDevice(j.second.idx))->GetDataRate().GetBitRate(); 
