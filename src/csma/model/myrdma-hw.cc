...
void MyRdmaHw::SetNode(Ptr<Node> node){
	m_node = node;
}
void MyRdmaHw::Setup(QpCompleteCallback cb){
	for (uint32_t i = 0; i < m_nic.size(); i++){
		Ptr<MyCsmaNetDevice> dev = m_nic[i].dev;
		if (dev == NULL)
			continue;
		// share data with NIC
		dev->m_rdmaEQ->m_qpGrp = m_nic[i].qpGrp;
		// setup callback 绑定了MyCsmaNetDevice的Receive函数们
		dev->m_rdmaReceiveCb = MakeCallback(&MyRdmaHw::Receive, this);
		dev->m_rdmaLinkDownCb = MakeCallback(&MyRdmaHw::SetLinkDown, this);
		dev->m_rdmaPktSent = MakeCallback(&MyRdmaHw::PktSent, this);
		// config NIC
		dev->m_rdmaEQ->m_rdmaGetNxtPkt = MakeCallback(&MyRdmaHw::GetNxtPacket, this);
	}
	// setup qp complete callback
	m_qpCompleteCallback = cb;
}

uint32_t MyRdmaHw::GetNicIdxOfQp(Ptr<MyRdmaQueuePair> qp){
...
uint64_t MyRdmaHw::GetQpKey(uint32_t dip, uint16_t sport, uint16_t pg){
	return ((uint64_t)dip << 32) | ((uint64_t)sport << 16) | (uint64_t)pg;
}
Ptr<MyRdmaQueuePair> MyRdmaHw::GetQp(uint32_t dip, uint16_t sport, uint16_t pg){
...
void MyRdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	// create qp
	Ptr<MyRdmaQueuePair> qp = CreateObject<MyRdmaQueuePair>(pg, sip, dip, sport, dport);
	qp->SetSize(size);
	qp->SetWin(win);
...
void MyRdmaHw::DeleteQueuePair(Ptr<MyRdmaQueuePair> qp){
	// remove qp from the m_qpMap
	uint64_t key = GetQpKey(qp->dip.Get(), qp->sport, qp->m_pg);
	m_qpMap.erase(key);
}

Ptr<MyRdmaRxQueuePair> MyRdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, bool create){
	...
	if (create){
		// create new rx qp
		Ptr<MyRdmaRxQueuePair> q = CreateObject<MyRdmaRxQueuePair>();
		// init the qp
		q->sip = sip;
		q->dip = dip;
		q->sport = sport;
...
uint32_t MyRdmaHw::GetNicIdxOfRxQp(Ptr<MyRdmaRxQueuePair> q){
...
void MyRdmaHw::DeleteRxQp(uint32_t dip, uint16_t pg, uint16_t dport){
	uint64_t key = ((uint64_t)dip << 32) | ((uint64_t)pg << 16) | (uint64_t)dport;
	m_rxQpMap.erase(key);
}

int MyRdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	uint8_t ecnbits = ch.GetIpv4EcnBits();

	uint32_t payload_size = p->GetSize() - ch.GetSerializedSize();

	// TODO find corresponding rx queue pair
	Ptr<MyRdmaRxQueuePair> rxQp = GetRxQp(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport, ch.udp.pg, true);
	if (ecnbits != 0){
	...
	if (x == 1 || x == 2){ //generate ACK or NACK
		...
		newp->AddHeader(head);
		AddHeader(newp, 0x800, Ipv4Address(ch.dip), Ipv4Address(ch.sip));	// Attach eth header ipv4
		// send
...
int MyRdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch){ // 用于处理接收到的CNP（Congestion Notification Packet，拥塞通知包）
	// QCN on NIC
	...
	uint32_t i;
	// get qp。查找对应的QP
	Ptr<MyRdmaQueuePair> qp = GetQp(ch.sip, udpport, qIndex); 
	if (qp == NULL)
		std::cout << "ERROR: QCN NIC cannot find the flow\n";
	// get nic
	uint32_t nic_idx = GetNicIdxOfQp(qp); // 获取NIC索引
	Ptr<MyCsmaNetDevice> dev = m_nic[nic_idx].dev; // 根据NIC索引获取对应的网络设备
	// 初始化QP的速率
...
int MyRdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){ // 检测到性能下降后，设置轮询包。即实现agent功能
	uint16_t qIndex = ch.ack.pg;
	uint16_t port = ch.ack.dport;
	uint32_t seq = ch.ack.seq;
	uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
	int i;
	Ptr<MyRdmaQueuePair> qp = GetQp(ch.sip, port, qIndex);
	if (qp == NULL){
	...
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	Ptr<MyCsmaNetDevice> dev = m_nic[nic_idx].dev;
	if (m_ack_interval == 0)
	...
	//MyRdma NPA agent; agent是一个完全自主的系统，能够在长时间内独立运行，通常会牺牲延迟和成本来换取更好的任务性能
	if(m_agent_flag){ // 如果是agent_node
		...
		if (qp->npa.m_maxRtt > 10000 && (Simulator::Now().GetTimeStep() % 1000000 > 900000)){ // 当历史最大RTT超过 10000，并且在特定时间窗口内：触发轮询包发送
			...
			AddHeader(p, 0x800, Ipv4Address(ch.dip), Ipv4Address(ch.sip));// 添加以太网头（0x800 表示IPv4协议）
			// 发送轮询数据包
...
int MyRdmaHw::ReceiveSignal(Ptr<Packet> p, CustomHeader &ch){
	if(!m_analysis_flag)
		return 0;
...
int MyRdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch){
...
int MyRdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<MyRdmaRxQueuePair> q, uint32_t size){
...
void MyRdmaHw::AddHeader (Ptr<Packet> p, uint16_t protocolNumber){
	PppHeader ppp;
	ppp.SetProtocol (EtherToPpp (protocolNumber));
	p->AddHeader (ppp);
	
}
void MyRdmaHw::AddHeader (Ptr<Packet> p, uint16_t protocolNumber, Address src, Address dst){
	EthernetHeader eth;
...
uint16_t MyRdmaHw::EtherToPpp (uint16_t proto){
	switch(proto){
...
void MyRdmaHw::RecoverQueue(Ptr<MyRdmaQueuePair> qp){
	qp->snd_nxt = qp->snd_una;
}

void MyRdmaHw::QpComplete(Ptr<MyRdmaQueuePair> qp){
...
void MyRdmaHw::SetLinkDown(Ptr<MyCsmaNetDevice> dev){
	printf("RdmaHw: node:%u a link down\n", m_node->GetId());
}

void MyRdmaHw::AddTableEntry(Ipv4Address &dstAddr, uint32_t intf_idx){
	uint32_t dip = dstAddr.Get();
	m_rtTable[dip].push_back(intf_idx);
}

void MyRdmaHw::ClearTable(){
	m_rtTable.clear();
}

void MyRdmaHw::RedistributeQp(){
	// clear old qpGrp
	for (uint32_t i = 0; i < m_nic.size(); i++){
		if (m_nic[i].dev == NULL)
			continue;
	...
	// redistribute qp
	for (auto &it : m_qpMap){
		Ptr<MyRdmaQueuePair> qp = it.second;
		uint32_t nic_idx = GetNicIdxOfQp(qp);
		m_nic[nic_idx].qpGrp->AddQp(qp);
		// Notify Nic
...
Ptr<Packet> MyRdmaHw::GetNxtPacket(Ptr<MyRdmaQueuePair> qp){
	...
	// add eth header
	AddHeader(p, 0x0800, Ipv4Address(qp->sip), Ipv4Address(qp->dip) );
	// update state
...
void MyRdmaHw::PktSent(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> pkt, Time interframeGap){
	qp->lastPktSize = pkt->GetSize();
	UpdateNextAvail(qp, interframeGap, pkt->GetSize());
}

void MyRdmaHw::UpdateNextAvail(Ptr<MyRdmaQueuePair> qp, Time interframeGap, uint32_t pkt_size){
	Time sendingTime;
	if (m_rateBound)
...
void MyRdmaHw::ChangeRate(Ptr<MyRdmaQueuePair> qp, DataRate new_rate){
	#if 1
 ...
 *****************************/
void MyRdmaHw::UpdateAlphaMlx(Ptr<MyRdmaQueuePair> q){
	#if PRINT_LOG
...
void MyRdmaHw::ScheduleUpdateAlphaMlx(Ptr<MyRdmaQueuePair> q){
	q->mlx.m_eventUpdateAlpha = Simulator::Schedule(MicroSeconds(m_alpha_resume_interval), &MyRdmaHw::UpdateAlphaMlx, this, q);
}

void MyRdmaHw::cnp_received_mlx(Ptr<MyRdmaQueuePair> q){
...
void MyRdmaHw::CheckRateDecreaseMlx(Ptr<MyRdmaQueuePair> q){
	...
	if (q->mlx.m_decrease_cnp_arrived){
		...
		q->mlx.m_rpTimer = Simulator::Schedule(MicroSeconds(m_rpgTimeReset), &MyRdmaHw::RateIncEventTimerMlx, this, q);
		#if PRINT_LOG
		printf("(%.3lf %.3lf)\n", q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
		#endif
	}
}
void MyRdmaHw::ScheduleDecreaseRateMlx(Ptr<MyRdmaQueuePair> q, uint32_t delta){
	q->mlx.m_eventDecreaseRate = Simulator::Schedule(MicroSeconds(m_rateDecreaseInterval) + NanoSeconds(delta), &MyRdmaHw::CheckRateDecreaseMlx, this, q);
}

void MyRdmaHw::RateIncEventTimerMlx(Ptr<MyRdmaQueuePair> q){
	q->mlx.m_rpTimer = Simulator::Schedule(MicroSeconds(m_rpgTimeReset), &MyRdmaHw::RateIncEventTimerMlx, this, q);
	RateIncEventMlx(q);
	q->mlx.m_rpTimeStage++;
}
void MyRdmaHw::RateIncEventMlx(Ptr<MyRdmaQueuePair> q){
...
void MyRdmaHw::FastRecoveryMlx(Ptr<MyRdmaQueuePair> q){
	#if PRINT_LOG
...
void MyRdmaHw::ActiveIncreaseMlx(Ptr<MyRdmaQueuePair> q){
	#if PRINT_LOG
...
void MyRdmaHw::HyperIncreaseMlx(Ptr<MyRdmaQueuePair> q){
	#if PRINT_LOG
 ...
 * High Precision CC
 ***********************/
void MyRdmaHw::HandleAckHp(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
...
void MyRdmaHw::UpdateRateHp(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
...
void MyRdmaHw::FastReactHp(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	if (m_fast_react)
 ...
 * TIMELY
 *********************/
void MyRdmaHw::HandleAckTimely(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	uint32_t ack_seq = ch.ack.seq;
	// update rate
...
void MyRdmaHw::UpdateRateTimely(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool us){
	uint32_t next_seq = qp->snd_nxt;
	uint64_t rtt = Simulator::Now().GetTimeStep() - ch.ack.ih.ts;
	bool print = !us;
...
void MyRdmaHw::FastReactTimely(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
}

/**********************
 * DCTCP
 *********************/
void MyRdmaHw::HandleAckDctcp(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	uint32_t ack_seq = ch.ack.seq;
	uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
 ...
 * HPCC-PINT
 ********************/
void MyRdmaHw::SetPintSmplThresh(double p){
       pint_smpl_thresh = (uint32_t)(65536 * p);
}
void MyRdmaHw::HandleAckHpPint(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
       uint32_t ack_seq = ch.ack.seq;
...
void MyRdmaHw::UpdateRateHpPint(Ptr<MyRdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
       uint32_t next_seq = qp->snd_nxt;
